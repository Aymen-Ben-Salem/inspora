...
export function NewsletterForm({
  ...
  if (status === "success") {
    return (
      <p className={compact ? "text-center text-xs text-[#777]" : "text-sm text-[#767676]"} role="status">
        {message}
      </p>
    );
  }
  ...
  return (
    <form
      className={`min-w-0 items-center ${compact ? "grid w-full grid-cols-[minmax(0,1fr)_82px] gap-2 sm:flex xl:gap-3" : "flex w-full max-w-[429px] gap-0"}`}

...
function createR2Client(environment: ReturnType<typeof loadProductionMediaEnvironment>) {
  return new S3Client({
    region: "auto",
...
async function listProductionStorageKeys(
  client: S3Client,
  environment: ReturnType<typeof loadProductionMediaEnvironment>,
) {
...
async function main() {
  ...
  const options = parseProductionExecutionOptions();
  ...
  const availableStorageKeys = await listProductionStorageKeys(client, environment);
  ...
  console.log(
    `${options.execute ? "Executing" : "Dry run:"} ${runnableCandidates.length} production video preview(s).`,
  );
  ...
  for (const candidate of runnableCandidates) {
    console.log(
      `- ${candidate.resourceType}:${candidate.id} source=${candidate.sourceKind === "development-r2" ? "production-r2" : new URL(candidate.sourceUrl).hostname + " (read-only)"} -> ${buildDevelopmentPreviewStorageKey(candidate)}`,
    );
  }

  if (!options.execute || runnableCandidates.length === 0) return;
  const tempRoot = await mkdtemp(join(tmpdir(), "inspora-production-video-previews-"));
  let failures = 0;
  ...
  if (failures > 0) {
    throw new Error(`${failures} production video preview(s) failed.`);
  }
}

main().catch((error) => {
  console.error("Production video preview migration failed.", error);
  process.exitCode = 1;
});
